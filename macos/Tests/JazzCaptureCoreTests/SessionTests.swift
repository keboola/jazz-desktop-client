...
final class SessionTests: XCTestCase {
    func testParseTimestampToleratesFractionalSeconds() {
        XCTAssertNil(JazzSession.parseTimestamp(nil))
        XCTAssertNil(JazzSession.parseTimestamp(""))
        XCTAssertNil(JazzSession.parseTimestamp("not-a-date"))
        XCTAssertNotNil(JazzSession.parseTimestamp("2026-06-09T13:45:16Z"))
        // Keboola timestamps carry fractional seconds, which the default ISO8601DateFormatter rejects.
        XCTAssertNotNil(JazzSession.parseTimestamp("2026-06-09T13:45:16.74865Z"))
    }
