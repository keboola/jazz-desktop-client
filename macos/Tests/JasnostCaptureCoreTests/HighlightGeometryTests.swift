...
final class HighlightGeometryTests: XCTestCase {
    ...
    func testSingleDisplayFlip() {
        ...
        let local = HighlightGeometry.toLocal(
            axFrame: ax, primaryHeight: 900, windowFrame: CGRect(x: 0, y: 0, width: 1440, height: 900))
        // Cocoa bottom edge = 900 - (100 + 50) = 750; x unchanged; size unchanged.
        XCTAssertEqual(local, CGRect(x: 100, y: 750, width: 200, height: 50))
    }

    /// Element at the very top-left maps to the top of the Cocoa window.
    func testTopLeftElement() {
        let ax = CGRect(x: 0, y: 0, width: 80, height: 20)
        let local = HighlightGeometry.toLocal(
            axFrame: ax, primaryHeight: 900, windowFrame: CGRect(x: 0, y: 0, width: 1440, height: 900))
        XCTAssertEqual(local, CGRect(x: 0, y: 880, width: 80, height: 20))  // 900 - 20
    }
    ...
    func testSecondDisplayLeftOfPrimary() {
        ...
        let ax = CGRect(x: -1340, y: 100, width: 200, height: 50)  // on the left display
        let union = CGRect(x: -1440, y: 0, width: 2880, height: 900)
