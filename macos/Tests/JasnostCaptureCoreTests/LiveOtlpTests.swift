...
/// `JASNOST_LIVE_OTLP_ENDPOINT` (the full OTLP base URL incl. the path-embedded secret) so
...
final class LiveOtlpTests: XCTestCase {
    func testPostsSessionSpanAndCorrelatedLogs() async throws {
        ...
        guard var endpoint = env["JASNOST_LIVE_OTLP_ENDPOINT"], !endpoint.isEmpty else {
            throw XCTSkip("JASNOST_LIVE_OTLP_ENDPOINT not set; skipping live OTLP test")
        }
        ...
        let context = OtlpMapper.SessionContext(
            ...
            startedAt: startedAt,
            kind: "live-test",
            user: env["JASNOST_LIVE_USER"] ?? "live-test@jasnost.invalid"
        )
        let events = [
            ActivityEvent(
