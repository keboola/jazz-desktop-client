...
public struct JasnostSession: Decodable, Identifiable, Sendable, Equatable {
...
public enum SessionList {
    ...
    public static func decode(_ data: Data) -> [JasnostSession] {
        struct Envelope: Decodable { let sessions: [JasnostSession] }
    ...
    public static func dedupe(_ sessions: [JasnostSession]) -> [JasnostSession] {
        var bestById: [String: JasnostSession] = [:]
        var order: [String] = []
        for s in sessions {
