...
final class DeviceTokenRenewer {
    ...
    private func commit(
        ...
        if grant.renewAfterSeconds == nil {
            NSLog(
                "jazz: device token renewed without a server lead time; "
        ...
        NSLog(
            "jazz: device token renewed: tokenId=%@ expiresAt=%@ nextRenewal=%@",
            grant.tokenId,
            grant.expiresAt,
    ...
    private func handle(
        ...
        guard case let .retryable(reason) = disposition else {
            ...
            backoffUntil = nil
            NSLog(
                "jazz: device token renewal stopped: tokenId=%@ outcome=%@",
                tokenId ?? "unknown",
                "\(disposition)")
        ...
        observedTokenId = tokenId
        NSLog(
            "jazz: device token renewal retrying: tokenId=%@ attempt=%d nextAttempt=%@",
            tokenId ?? "unknown",
            consecutiveFailures,
