...
/// Embeds the hosted jazz review Data App (the React SPA: timeline + clarify + L4 + BDM
...
/// messages the page posts to `window.webkit.messageHandlers.jazz` (a known set of `type`s).
///
...
private struct WebCanvasRepresentable: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, WKScriptMessageHandler, WKNavigationDelegate {
        ...
        func webViewWebContentProcessDidTerminate(_ webView: WKWebView) {
            NSLog("jazz: web content process terminated; reloading the canvas")
            webView.reload()
        }
    }
}
