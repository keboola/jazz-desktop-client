...
public sealed class CaptureJournal
{
        ...
        bool allowAlreadyAdmitted = false)
    {
        ...
        try
        {
            ...
            if (artifact?.Document is null
                || artifact.Document["kind"]?.GetValue<string>() != "screenshot"
    ...
    public static CaptureJournal Reopen(string root, string archiveId)
    {
        ...
        if (!HasNoExistingReparseAncestors(journal._root, journal._statePath))
        {
            throw JournalJson.Corrupt("capture journal state path crosses a reparse point");
        }
