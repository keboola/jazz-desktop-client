...
{
    private readonly Func<CancellationToken, Task> drain; private readonly CancellationTokenSource stop = new();
    private int running; private int nudged; private int attempt;
    public ScreenshotDeliveryScheduler(Func<CancellationToken, Task> drain) => this.drain = drain;
    ...
    {
        try { do { Interlocked.Exchange(ref nudged, 0); try { await drain(stop.Token).ConfigureAwait(false); attempt = 0; } catch (OperationCanceledException) when (stop.IsCancellationRequested) { break; } catch { attempt = Math.Min(attempt + 1, 6); try { await Task.Delay(TimeSpan.FromSeconds(1 << attempt), stop.Token).ConfigureAwait(false); } catch (OperationCanceledException) { break; } } } while (Volatile.Read(ref nudged) != 0 && !stop.IsCancellationRequested); }
