...
The versioned `jazz-desktop-client.issue-4.v1` profile contains all 33 issue scenarios. A terminal
...
6. Ensure every receipt time lies within the qualification run interval. Evidence for another
   ...
   validation.
7. Build the complete 33-result bundle and call `validate()`. A full terminal profile—whether it
...
- Coach scenarios require archive and Coach interaction summaries;
...
Changing a required kind changes the meaning of PASS and therefore requires a new profile version.
Do not edit an exported result to make a release pass. Repeat the physical scenario under a new
qualification run and retain both outcomes.
